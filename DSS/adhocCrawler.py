...
def get_all_website_links(url):
    ...
    while soup == '':
        try:
            soup = BeautifulSoup(requests.get(url, verify=True, timeout=5, headers=headers).content, "html.parser",
    ...
    if (config['features'][feature]['metadataFormat'] == "json" and property):
        ...
        if hasChanged:
            return filterByDatatype(jsonFile, config['features'][feature]['datatype'],
                                    config['features'][feature]['expectedValues'])
    return {}
...
def getByPostfix(feature, tag, html):
    ...
    if type(tag) != type(None):
        ...
        preTag = tag.find_next(config['features'][feature]['searchKeywords']['postfix']['tag'],
                               class_=config['features'][feature]['searchKeywords']['postfix']['cssClass'])
...
def getByInfix(feature, tag):
    global config
    if type(tag) != type(None):
        infix = tag.find(config['features'][feature]['searchKeywords']['infix']['tag'],
                         {"class": config['features'][feature]['searchKeywords']['infix']['cssClass']})
...
def is_not_crawled(features):
    ...
    for feature in features:
        if (config['allArray'] == 'True' and features[feature] == ["N/A"]) or (
                config['allArray'] == 'False' and features[feature] == "N/A"):
            return False
        if feature == "url":
