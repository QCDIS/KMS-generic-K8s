...
def getDatasetRecords__SeaDataNet_EDMED():
    with urllib.request.urlopen(
            'https://edmed.seadatanet.org/sparql/sparql?query=select+%3FEDMEDRecord+%3FTitle+where+%7B%3FEDMEDRecord+a+%3Chttp%3A%2F%2Fwww.w3.org%2Fns%2Fdcat%23Dataset%3E+%3B+%3Chttp%3A%2F%2Fpurl.org%2Fdc%2Fterms%2Ftitle%3E+%3FTitle+.%7D+&output=json&stylesheet=') as f:
...
def clean(doc):
    ...
    normalized = " ".join(
        lemma.lemmatize(word) for word in punc_free.split() if len(word) > 2 and EnglishTerm.check(word))
    return normalized
        ...
        for topic in lsttopics:
            ...
            if similarity >= acceptedSimilarityThreshold:
                lstEssentialVariables.append(
                    variable) if variable not in lstEssentialVariables else lstEssentialVariables
...
def datasetProcessing_ICOS(datasetURL):
        ...
        if '<script type="application/ld+json">' in str(script):
            ...
            for metadata_property in metadataStar_object:
                ...
                if (
                    ...
                    txtVal = flattenValue.replace("[", "").replace("]", "").replace("'", "").replace("\"", "").replace(
                        "\"\"", "").replace("None", "")
                    if txtVal != "":
                ...
                indexFile.write("\"" + str(metadata_property) + "\" :" +
                                flattenValue.replace("']", "\"]").replace("['", "[\"").replace("',", "\",").replace(
                                    ", '", ", \"").replace("\"\"", "\"")
                                + extrachar)
                CSVvalue = CSVvalue + flattenValue.replace(",", "-").replace("[", "").replace("]", "").replace("'",
                                                                                                               "").replace(
                    "\"", "") + ","
    ...
    indexFile.write("}")
    indexFile.close()
    Precision, Recall, Accuracy, F = metadataRecord_similarity_evaluation(indexfname + ".json", 'EssentialVariables',
                                                                          ['description', 'keywords', 'abstract'],
                                                                          essentialVariables)
    CSVvalue = CSVvalue + str(Precision) + "," + str(Recall) + "," + str(Accuracy) + "," + str(F) + ","
    Precision, Recall, Accuracy, F = metadataRecord_similarity_evaluation(indexfname + ".json", 'potentialTopics',
                                                                          ['description', 'keywords', 'abstract'],
                                                                          lstKeywords.split(","))
    CSVvalue = CSVvalue + str(Precision) + "," + str(Recall) + "," + str(Accuracy) + "," + str(F) + ","
    logfile.write(CSVvalue)
...
def datasetProcessing_SeaDataNet_EDMED(datasetURL):
    ...
    for metadata_property in metadataStar_object:
        ...
        if metadata_property in mapping:
            ...
            if (
                    metadata_property == "description" or metadata_property == "keywords" or metadata_property == "abstract"):
                txtVal = str(value).replace("[", "").replace("]", "").replace("'", "").replace("\"", "").replace("\"\"",
                                                                                                                 "").replace(
                    "None", "")
                if txtVal != "":
                    originalValues.append(txtVal)
            ...
            CSVvalue = CSVvalue + str(value).replace(",", "-").replace("[", "").replace("]", "").replace("'",
                                                                                                         "").replace(
                "\"", "") + ","
        else:
...
def getContextualText_SeaDataNet_CDI(JSON):
    ...
    contextualText = deep_search(
        ["Data set name", "Discipline", "Parameter groups", "Discovery parameter", "GEMET-INSPIRE themes"], JSON)
...
def datasetProcessing_SeaDataNet_CDI_XML(datasetURL):
    ...
    with urllib.request.urlopen(datasetURL) as f:
        data = f.read().decode('utf-8')
    data = data.replace("xml:", "").replace("eml:", "").replace("namespace:", "").replace("xmlns:", "").replace("gmd:",
                                                                                                                "").replace(
        "gco:", "").replace("sdn:", "").replace("gml:", "").replace("gts:", "").replace("xlink:", "").replace("\"{", "")
